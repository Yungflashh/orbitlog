...
app.post("/register", async (req, res) => {
  try {
    ...
    if (response.rows.length > 0) {
      res.render("registerPage.ejs", {
        error: `Email "${response.rows[0].email}" has been used`,
      });
    } else if (password.length < 6) {
      res.render("registerPage.ejs", {
        error: `Password length has to be greater than or equal to 6`,
      });
    } else if (password != checkPassword) {
      res.render("registerPage.ejs", {
        error: `Your password characters needs to match`,
      });
    ...
    //   console.log(hashPassword);
      const newUser = await db.query(
        "INSERT INTO orbitlog (username, email, password) VALUES ($1, $2, $3) RETURNING *",
